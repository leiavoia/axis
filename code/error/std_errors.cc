...
TO DO:
---------------------------
- add a string to the bounds error
...
public:
	void SetVal(int x) {
		if (x<0 || x>10) {throw err_bounds_int(1,10,x);}
		else {n = x;}
		}
...
try { test.SetVal(13); } catch(err_bounds_int &error) {
	cout << "min: " << error.min << "\n";
	cout << "max: " << error.max << "\n";
	cout << "attempted: " << error.attempted << "\n";
	}

cout << "value is: " << test.GetValue() << "\n";
